...
CREATE FUNCTION [dbo].[Student.Profile] (@Person_id char(13),@OBJ_attribt char(30) )  
RETURNS varchar(100)
--- Frank Mi
--- March 14, 2002
--- @perason_id  
...
AS  
   ...
   if @OBJ_attribt ='NumberF'
    ...
    else if @OBJ_attribt ='NumberF2'
           set @rValue = (select top 1 left(student_no,3) +space(2)+ substring(student_no,4,3) + space(2) + right(student_no,3)
                          from persons where person_id =@Person_id)
    ...
    else if @OBJ_attribt ='Sheltered'
    	 	begin  set @rValue = (select top 1 isnull(sheltered_student_flag_10,'0') from persons where person_id = @person_id) end
	...
	else if @OBJ_attribt ='SchoolCode'
     	--	declare @rValue varchar(2)
  	 	begin 
			set @rValue =  ( select top 1 school_code from student_registrations
				          where person_id = @person_id  and left(school_code,1) ='0' 
				          order by school_year DESC , status_indicator_code  )	 	 
         end
	else if @OBJ_attribt ='SchoolYear'
     	--	declare @rValue varchar(2)
  	 	begin set @rValue =  ( select top 1 school_year from student_registrations
				          where person_id = @person_id  and left(school_code,1) ='0' 
				          order by school_year DESC , status_indicator_code  )	 	 
        end
	else if @OBJ_attribt ='Gender'
     	--	declare @rValue varchar(2)
  		begin
   ...
   else if @OBJ_attribt ='MPCellPhone'   
  	 	begin      
               set @ReleshipID = dbo.personReleID(@person_id,'Father')
	            set @rValue =    dbo.PersonPhoneshort(@ReleshipID,'Cell')      				 
                 set @rValue = isnull(@rValue, ' ')        
	     end       
   else if @OBJ_attribt ='DearTitle'   
  	 	    begin 
      		         set @rValue =  'Parent / Guardian'  	 
	        end     

	else if @OBJ_attribt ='Mother'   
  	 	begin      
	         set @ReleshipID = dbo.personReleID(@person_id,'Mother')  
      		set @rValue =  dbo.personName(@ReleshipID, 'Full') 		 
		     set @rValue = isnull(@rValue, ' ')        
	     end     
	else if @OBJ_attribt ='MotherS'   
  	 	begin      
	          set @ReleshipID = dbo.personReleID(@person_id,'Mother')  
      		  set @rValue =  dbo.personName(@ReleshipID, 'SurName') 		 
		       set @rValue = isnull(@rValue, ' ')        
	     end   
	else if @OBJ_attribt ='MotherF'   
  	 	begin      
	         set @ReleshipID = dbo.personReleID(@person_id,'Mother')  
      		   set @rValue =  dbo.personName(@ReleshipID, 'PFirst') 		 
		     set @rValue = isnull(@rValue, ' ')        
	...
	else if @OBJ_attribt ='MotherPhone'   
  	 	begin      
	         set @ReleshipID = dbo.personReleID(@person_id,'Mother')
	         set @rValue =    dbo.PersonPhoneshort(@ReleshipID,'Business')      				 
		      set @rValue = isnull(@rValue, ' ')        
	     end    
	else if @OBJ_attribt ='MotherPhoneB'   
  	 	begin      
	           set @ReleshipID = dbo.personReleID(@person_id,'Mother')
	             set @rValue =    dbo.personphonenumber(@ReleshipID,'Business')      				 
		          set @rValue = isnull(@rValue, ' ')        
	    end    
    else if @OBJ_attribt ='FPRelationship'   
  	 	begin      
                set @ReleshipID = dbo.personReleID(@person_id,'Mother')                                   
   ...
   else if @OBJ_attribt ='FPCellPhone'   
  	 	begin      
             set @ReleshipID = dbo.personReleID(@person_id,'Mother')
	          set @rValue =    dbo.PersonPhoneshort(@ReleshipID,'Cell')      				 
                set @rValue = isnull(@rValue, ' ')        
	...
	else if @OBJ_attribt ='DoctorName'   
  	 	begin     
			 set @ReleshipID = (select doctor_id from persons where person_id = @person_id)  -----dbo.personReleID(@person_id,'Doctor')  --eva au
	...
	else if @OBJ_attribt ='DoctorPhone'   
  	 	begin      
			set @ReleshipID = (select doctor_id from persons where person_id = @person_id)  -----dbo.personReleID(@person_id,'Doctor')  --eva au
   ...
   else if @OBJ_attribt ='Emergency'   
  	 	begin      
	                        set @ReleshipID = dbo.personReleIDemerg(@person_id,'Emergency')
	                        set @rValue =  dbo.personName(@ReleshipID, 'Full' )		 
		          set @rValue = isnull(@rValue, ' ')        		 
	     end    
    else if @OBJ_attribt ='EmergencyS'   
  	 	begin      
	                        set @ReleshipID = dbo.personReleIDemerg(@person_id,'Emergency')
	                        set @rValue =  dbo.personName(@ReleshipID, 'SurName' )		 
		          set @rValue = isnull(@rValue, ' '  )        		 
	              end    
  else if @OBJ_attribt ='EmergencyF'   
  	 	begin      
	                        set @ReleshipID = dbo.personReleIDemerg(@person_id,'Emergency')
	                        set @rValue =  dbo.personName(@ReleshipID, 'PFirst' )		 
		          set @rValue = isnull(@rValue, '  ')        		 
	              end    
   else if @OBJ_attribt ='EmergencyPhone'   
  	 	begin      
	                        set @ReleshipID = dbo.personReleIDemerg(@person_id,'Emergency')
	                       set @rValue =   dbo.PersonPhoneshort(@ReleshipID,'Business')      				 
		          set @rValue = isnull(@rValue, ' ')        		 
...
---
	else if @OBJ_attribt ='EmergencyPhonehome'   
  	 	begin      
	                        set @ReleshipID = dbo.personReleIDemerg(@person_id,'Emergency')
	                       set @rValue =   dbo.PersonPhoneshort(@ReleshipID,'Home')      				 
		          set @rValue = isnull(@rValue, ' ')        		 
	    end  
    else if @OBJ_attribt ='EmergencyRelationship'   
  	 	begin      
    ...
    else if @OBJ_attribt ='EmergencyCellPhone'   
  	 	begin      
	                       set @ReleshipID = dbo.personReleID(@person_id,'Emergency')
	                       set @rValue =   dbo.PersonPhoneshort(@ReleshipID,'Cell')                                   
		          set @rValue = isnull(@rValue, ' ')        		 
	              end    
	else if @OBJ_attribt ='Homeroom'   
  	       begin      
   ...
   else if @OBJ_attribt ='HomeroomNo'   
  	     begin      
               	select  	@school_code  = school_code, 
                    		@school_year = school_year, 
                    		@school_date = cast(school_date as smalldatetime)  
	...
	else if @OBJ_attribt ='Homeroomclass'   
  	      begin      
               	select  	@school_code  = school_code, 
                    		@school_year = school_year, 
                    		@school_date = cast(school_date as smalldatetime)  
   ...
   else if @OBJ_attribt ='ReadingTestDate'
	       begin	 
   ...
   else if @OBJ_attribt = 'TeacherName'     	
       begin
	 		select  	@school_code  = school_code, 
                    			@school_year = school_year, 
                    			@school_date = cast(school_date as smalldatetime)  
   ...
   else if @OBJ_attribt = 'Cetificate'  
        begin     	
