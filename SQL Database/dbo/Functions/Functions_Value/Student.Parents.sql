...
CREATE FUNCTION [dbo].[Student.Parents](@person_id char(13),@vType char(20) )
RETURNS varchar(50) 
--- Frank Mi
--- May 20 2003.  enhance function at July 10 2015
      ...
      if @FatherID is null 
         begin 
            if @MotherID is null
			 ...
			 where 	student_person_id = @person_id and  access_to_records_flag ='x'
		         order by primary_contact_priority, start_date desc)
                   set @rValue = dbo.PersonPhoneNumber(@GuardianID,'Internet') 
               end
            else
               set @rValue =  dbo.PersonPhoneNumber(@MotherID,'Internet') 
        end
      else
         begin
            if @MotherID is null
               set @rValue = dbo.PersonPhoneNumber(@FatherID,'Internet')  -- dbo.PersonName(@FatherID,'Full') 
            else
               set @rValue = isnull(dbo.PersonPhoneNumber(@MotherID,'Internet'),'')
         end   
     set @rValue = isnull(@rValue,'') 
